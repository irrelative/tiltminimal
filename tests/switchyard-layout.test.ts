...
describe('Switchyard open shot geometry', () => {
  it('keeps scoring obstacles above the open lower middle and a ball-sized center drain', () => {
    ...
    expect(
      board.posts.every(
        (post) => post.x < 240 || post.x > 660 || post.y > 1080 || post.y < 760,
      ),
    ).toBe(true);
    expect(board.guides.every((guide) => guide.plane !== 'raised')).toBe(true);
  ...
  it('tags real switches with their physical ball and preserves identity after primary drain', () => {
    ...
    expect(r.events).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'rollover-hit', index: 0, ballId: 7 }),
        expect.objectContaining({ type: 'rollover-hit', index: 2, ballId: 8 }),
      ]),
    );
    s = r.state;
