...
const { lower, shooter, ...foundation } = createFoundation('switchyard', 1);
...
const parts = composeAssemblies(
  lower,
  shooter,
  createBank('switchyard-cargo', { x: 270, y: 550 }, { x: 40, y: 60 }, 2),
  createBank('switchyard-signal', { x: 640, y: 550 }, { x: -40, y: 60 }, 2),
  dispatch,
);
parts.guides!.push(
  ...
  parts.routes!.push({
    ...
    goals: [
      ...order.map((index) => ({
        ...
        position: parts.rollovers![index].position as { x: number; y: number },
      })),
...
export const switchyardTable = compileBuiltInBoardLayout(
  {
    ...foundation,
    ...parts,
    name: 'Switchyard',
