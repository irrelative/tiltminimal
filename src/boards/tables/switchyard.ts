import {
  createSaucerPocketAssembly,
  createTargetBankAssembly,
} from '../assemblies';
import { arc, rail } from '../assemblies/shared';
import { compileBuiltInBoardLayout } from '../layout-compiler';
import { createFoundation, composeAssemblies } from './table-foundation';
import { switchyardRulesScript } from './switchyard-rules';

// Switchyard owns its shot geometry; Just One More retains the shared fan layout.
function createSwitchyardTable() {
  const id = 'switchyard';
  const { lower, shooter, ...foundation } = createFoundation(id, 1);
  // The launcher joins an uninterrupted upper orbit. Remove the lane-stop and
  // dividers, retaining the cabinet arch, tangent walls and shooter gate.
  shooter.guides = shooter.guides!.slice(0, 4);
  shooter.rollovers = [];
  shooter.routes[0].goals = [
    { type: 'region', min: { x: 20, y: 260 }, max: { x: 200, y: 700 } },
    { type: 'region', min: { x: 200, y: 720 }, max: { x: 710, y: 1130 } },
  ];
  const dispatch = createSaucerPocketAssembly({
    id: `${id}-saucer`,
    center: { x: 360, y: 360 },
    radius: 28,
    wallRadius: 48,
    throatLength: 35,
    mouthDepth: 100,
    mouthHalfWidth: 80,
    score: 2000,
    holdSeconds: 0.5,
    ejectSpeed: 640,
    ejectAngle: 1.25,
    approachSpeed: 1050,
    returnRegion: {
      type: 'region',
      min: { x: 380, y: 510 },
      max: { x: 500, y: 660 },
    },
  });
  const parts = composeAssemblies(
    lower,
    shooter,
    createTargetBankAssembly({
      id: 'switchyard-cargo',
      first: { x: 260, y: 560 },
      step: { x: 35, y: 60 },
      standupCount: 0,
      dropCount: 3,
      targetWidth: 50,
      targetHeight: 16,
      backingOffset: { x: 55, y: -25 },
      backingExtension: 18,
      standupScore: 500,
      dropScore: 500,
      returnRegion: {
        type: 'region',
        min: { x: 180, y: 770 },
        max: { x: 720, y: 1130 },
      },
    }),
    createTargetBankAssembly({
      id: 'switchyard-signal',
      first: { x: 490, y: 490 },
      step: { x: 10, y: 65 },
      standupCount: 2,
      targetWidth: 38,
      targetHeight: 12,
      backingOffset: { x: 25, y: -15 },
      backingExtension: 10,
      standupScore: 500,
      returnRegion: {
        type: 'region',
        min: { x: 480, y: 700 },
        max: { x: 780, y: 1100 },
      },
    }),
    dispatch,
  );
  parts.guides!.push(
    // End the inner arch before the gate: the 57-unit opening lets a slow
    // returning ball roll off the gate into the orbit instead of wedging
    // against a continuous inner rail. The one-way gate still closes the lane.
    arc({ x: 478, y: 506 }, 410, Math.PI, Math.PI * 2 - 0.94),
    rail({ x: 12, y: 506 }, { x: 12, y: 1380 }),
    rail({ x: 12, y: 670 }, { x: 120, y: 800 }),
    rail({ x: 888, y: 670 }, { x: 780, y: 800 }),
    rail({ x: 68, y: 506 }, { x: 100, y: 650 }),
    arc({ x: 450, y: 506 }, 260, Math.PI, Math.PI * 1.5),
    ...Array.from({ length: 16 }, (_, i) => {
      const point = (t: number) => ({
        x: 450 + 780 * t - 390 * t * t,
        y: 246 + 260 * t * t,
      });
      return rail(point(i / 16), point((i + 1) / 16));
    }),
    rail({ x: 190, y: 506 }, { x: 190, y: 600 }),
    rail({ x: 190, y: 600 }, { x: 280, y: 740 }),
    rail({ x: 840, y: 506 }, { x: 840, y: 600 }),
    rail({ x: 840, y: 600 }, { x: 760, y: 690 }),
    // East's short horseshoe returns through its own inner leg.
    arc({ x: 650, y: 430 }, 110, Math.PI, Math.PI * 2),
    arc({ x: 650, y: 430 }, 50, Math.PI, Math.PI * 2),
    rail({ x: 760, y: 430 }, { x: 760, y: 690 }),
    rail({ x: 700, y: 430 }, { x: 700, y: 650 }),
    rail({ x: 700, y: 650 }, { x: 650, y: 750 }),
    rail({ x: 540, y: 430 }, { x: 540, y: 620 }),
    rail({ x: 600, y: 430 }, { x: 600, y: 620 }),
  );
  parts.spinners = [145].map((x) => ({
    position: { x, y: 690 },
    length: 60,
    angle: 0,
    thickness: 10,
    material: 'metalGuide',
    score: 100,
  }));
  // Non-solid switches identify complete travel, independently for each ball.
  parts.rollovers = [
    { position: { x: 140, y: 550 }, radius: 35, score: 0 },
    { position: { x: 450, y: 170 }, radius: 60, score: 0, approachAngle: 0 },
    { position: { x: 863, y: 550 }, radius: 18, score: 0 },
    // Skill exit below the gate, then an overshoot switch across the arch.
    {
      position: { x: 738, y: 225 },
      radius: 18,
      score: 0,
      approachAngle: 2.4,
      approachDistance: 50,
    },
    { position: { x: 677, y: 116 }, radius: 18, score: 0, approachAngle: 0.47 },
    { position: { x: 730, y: 610 }, radius: 18, score: 0 },
    {
      position: { x: 650, y: 350 },
      radius: 18,
      score: 0,
      approachAngle: Math.PI,
      approachDistance: 60,
    },
    { position: { x: 570, y: 570 }, radius: 18, score: 0 },
  ];
  for (const { name, x, y, order } of [
    { name: 'west-orbit', x: 145, y: 740, order: [0, 1, 2] },
    { name: 'east-loop', x: 730, y: 740, order: [5, 6, 7] },
    { name: 'east-loop-reverse', x: 570, y: 650, order: [7, 6, 5] },
  ]) {
    parts.routes!.push({
      id: `${id}-${name}`,
      start: {
        type: 'feed',
        position: { x, y },
        velocities: [2000, 2200, 2400].map((y) => ({ x: 0, y: -y })),
      },
      goals: [
        ...order.map((index) => ({
          type: 'event' as const,
          event: 'rollover-hit' as const,
          position: parts.rollovers![index].position as {
            x: number;
            y: number;
          },
        })),
        // Reverse East spills above the right inlane/sling; other traversals
        // return farther inward. None of these exits promises an auto-catch.
        name === 'east-loop-reverse'
          ? { type: 'region', min: { x: 620, y: 720 }, max: { x: 780, y: 850 } }
          : {
              type: 'region',
              min: { x: 180, y: 800 },
              max: { x: 720, y: 1130 },
            },
      ],
      timeoutSeconds: 6,
    });
  }
  for (const x of [445, 450, 455])
    parts.routes!.push({
      id: `${id}-center-drain-${x}`,
      start: {
        type: 'feed',
        position: { x, y: 1270 },
        velocities: [300, 650].map((y) => ({ x: 0, y })),
      },
      goals: [{ type: 'drain' }],
      avoidFlippers: true,
      timeoutSeconds: 3,
    });
  return compileBuiltInBoardLayout(
    {
      ...foundation,
      ...parts,
      name: 'Switchyard',
      themeId: 'switchyard',
      rulesScript: switchyardRulesScript,
    },
    { snapToGrid: false },
  );
}

export const switchyardTable = createSwitchyardTable();
